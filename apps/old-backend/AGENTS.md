...
**New Database Table:**
1. Create table definition in module: `modules/<module-name>/tables/<entity>.table.ts` extending `BaseTable` with snake_case columns
   - Example: `modules/users/users/users.table.ts`
   - Example: `modules/journal-entries/tables/journal_entries.table.ts`
   - Tables are organized by module/feature to keep related code together
